...
---@class DeathKnightBloodV3Config
local Config = {
...
---@return DeathKnightBloodV3Config
...
---@param object DeathKnightBloodV3Config another config
...
---Determine state
---@param cr AmstLibCombatRotation
---@param cfg DeathKnightBloodV3Config config
---@return void
...
---@class DeathKnightBloodV3Rune
local DeathKnightBloodV3Rune = {
    type = 0,
    isReady = false,
    cooldownStart = 0,
    cooldownDuration = 0,
...
---@return DeathKnightBloodV3Rune
function DeathKnightBloodV3Rune:new(type, isReady, cooldownStart, cooldownDuration)
    local o = {
        type = type,
        isReady = isReady,
...
---@class DeathKnightBloodV3Runes
local DeathKnightBloodV3Runes = {
    ---@type DeathKnightBloodV3Rune[]
    runes = {}
}

---@return DeathKnightBloodV3Runes
function DeathKnightBloodV3Runes:new()
    local runes = {}
    for id = 1, 6 do
        local cdStart, cdDur, isReady = GetRuneCooldown(id)
        table.insert(runes, DeathKnightBloodV3Rune:new(GetRuneType(id), isReady, cdStart, cdDur))
    end
    local o = {
        runes = runes,
    }
...
function DeathKnightBloodV3Runes:willBeReadyOrReserve(reserveId, type, startPeriod, endPeriod)
...
function DeathKnightBloodV3Runes:canConsume(reserveId, blood, frost, unholy)
...
function DeathKnightBloodV3Runes:summary()
    local runesSummary = {
        [RUNE_TYPE_BLOOD] = 0,
        [RUNE_TYPE_FROST] = 0,
        [RUNE_TYPE_UNHOLY] = 0,
...
---@class DeathKnightBloodV3Rotation
local Rotation = {
    ---@type DeathKnightBloodV3Config
    cfg = nil,
    ---@type DeathKnightBloodV3State
    state = nil,
    ---@type AmstLibCombatRotation
    cr = nil,
...
---@param cfg DeathKnightBloodV3Config
---@param state DeathKnightBloodV3State
---@param cr AmstLibCombatRotation
---@param trinketer AmstLibTrinketer
---@return DeathKnightBloodV3Rotation
function Rotation:new(cfg, state, cr, trinketer)
    ---@type DeathKnightBloodV3Rotation
    local o = {
        cfg = cfg,
        state = state,
        cr = cr,
...
function Rotation:execute()
    ...
    local runes = DeathKnightBloodV3Runes:new()
    ...
    if shouldUseBloodFillers and runes:canConsume("", 1, 0, 0) then
        ...
        if amstlib.CONST.CLASSIC.SPELL_KNOWN.heartStrike then
            if GMR.IsCastable(amstlib.CONST.CLASSIC.SPELL.heartStrike, "target")
                and GMR.IsSpellInRange(amstlib.CONST.CLASSIC.SPELL.heartStrike, "target")
            then
            ...
            if GMR.IsCastable(amstlib.CONST.CLASSIC.SPELL.bloodStrike, "target")
                and GMR.IsSpellInRange(amstlib.CONST.CLASSIC.SPELL.bloodStrike, "target")
            then
    ...
    if amstlib.CONST.CLASSIC.SPELL_KNOWN.deathStrike then
        if GMR.IsCastable(amstlib.CONST.CLASSIC.SPELL.deathStrike, "target")
            and GMR.IsSpellInRange(amstlib.CONST.CLASSIC.SPELL.deathStrike, "target")
        then
        ...
        cr:initialize(
            ...
            function()
                return GMR.GetClass("player") == amstlib.CONST.CLASS.DEATHKNIGHT
                    and select(5, GetTalentInfo(1, 28)) == 2
            end,
            function()
