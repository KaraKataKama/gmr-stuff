...
function Rotation:execute()
    ...
    elseif self.state.hasGlyphOfDisease and (targetBloodPlagueDuration > 0 and targetBloodPlagueDuration < 3)
        and (targetFrostFeverDuration > 0 and targetFrostFeverDuration < 3)
    then
        needToCastPestilenceSpell = true
    end
    ...
    if not (needToCastPestilenceSpell and spellKnown.pestilence) then
        ...
        if self.cfg.useBloodFillersWithBloodRunesOnly and runes:summary()[RUNETYPE_BLOOD] == 0 then
    ...
    elseif self.cfg.useObliterate and spellKnown.obliterate and GMR.IsCastable(spells.obliterate, "target") then
        self.dbgPrint("should use obliterate as a filler")
        GMR.Cast(spells.obliterate, "target")
        return
    else
        if self.cfg.usePlagueStrikeAsFiller and GMR.IsCastable(spells.plagueStrike, "target") and GMR.IsSpellInRange(spells.plagueStrike, "target") then
            self.dbgPrint("should use plague strike as a filler")
            GMR.Cast(spells.plagueStrike, "target")
            return
        end

        if self.cfg.useIcyTouchAsFiller and GMR.IsCastable(spells.icyTouch, "target") and GMR.IsSpellInRange(spells.icyTouch, "target") then
